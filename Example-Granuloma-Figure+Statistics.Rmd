...
```
# Code to Generate Density Plots from Extended Figure 2

#for a combined dataset df with per entry metadata Sample and Region (Tissue Subtype)
samples <- unique(df$Sample)
p <- list()
for (s in 1:length(samples)) {
  med_df <- cleaned[cleaned$Sample == samples[s], ] %>%
  group_by(Region) %>%
  summarize(median=median(Log_Ratio_MFI))
  
  
  p[[c]] <- ggplot(cleaned[cleaned$Sample == samples[s], ],  aes(x=Log_Ratio_MFI, fill = Region)) +  geom_density(alpha = 0.5) +  scale_fill_manual(values=c( "#43a2ca", "#de2d26")) +
            theme_bw() + theme(axis.title.y = element_blank()) + ylab("") + geom_vline(data=med_df, aes(xintercept=median, color=Region),
             linetype="dashed") + scale_color_manual(values=c("#43a2ca", "#de2d26"))
}
...
aggregate(Red~Location, data = cat_3, var)
...
aggregate(Green~Location, data = cat_3, var)
...
kruskal.test(Red~Location, data = cat_3)
Kruskal-Wallis rank sum test
